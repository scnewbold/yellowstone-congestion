...
if(FALSE){
  ...
  if(TRUE){
    fig.name <- paste(output.path,'/sim-histograms-fullyear-v0',sep='')
...
if(FALSE){
  ...
  # pdf figure: simulation histograms
  if(TRUE){
    fig.name <- paste(output.path,'/sim-histograms-mayoct-v0',sep='')
    tikz(paste(fig.name,'.tex',sep=''),
         width      = 6.5,
         height     = 6.5,
         pointsize  = 12,
...
if(TRUE){
  ...
  BS                  <- 1000 # number of iterations for bootstrap
  ...
  if(TRUE){
    ...
    fig.name <- paste(output.path,'/cv-vs-y-v0',sep='')
    tikz(paste(fig.name,'.tex',sep=''),
         width      = 4.5,
         height     = 4.5,
         pointsize  = 12,
  ...
  # CONGESTION ESTIMATION:
  {
    ...
    if(TRUE){
      depv  <- log(Ytm)
      YYtm  <- Ytm/10^6
      outs  <- summary(lm(depv ~ D.yrs + D.mth + YYtm * open))
      theta.hat.v2 <- tail(outs$coeff,1)[1]
    }
    
    # pdf figure:
    {
      fig.name <- paste(output.path,'/congestion-peak-v0',sep='')
      tikz(paste(fig.name,'.tex',sep=''),
           width      = 5.5,
           height     = 5.5,
           pointsize  = 12,
  ...
  if(TRUE){
    ...
    for(bs in 1:BS){
      ...
      # Re-estimate congestion coefficient:
      {
        ...
        for(theta.k in theta.range){k <- k + 1
          ...
          outs   <- summary(lm(depv.k ~ D.yrs + D.mth))
          R2[k]  <- outs$adj.r.squared
        }
    ...
    # Table of bootstrap results:
    {
      ...
      cat('==========================================================================\n',file=out.file.name,append=TRUE)
      cat('Parameter                               Estimate      E[Bias]      E[s.e.]\n',file=out.file.name,append=TRUE)
      cat('--------------------------------------------------------------------------\n',file=out.file.name,append=TRUE)
      cat(sprintf('Congestion coefficient, $\\theta$   &  %10.2f & %10.5f & %10.5f\\\\\n',theta.hat,mean(theta.BS)-theta.hat,sd(theta.BS)),file=out.file.name,append=TRUE)
      cat('==========================================================================\n',file=out.file.name,append=TRUE)
    }
    
    # pdf figure: bootstrap histogram
    {
      fig.name <- paste(output.path,'/bootstrap-histogram-v0',sep='')
      tikz(paste(fig.name,'.tex',sep=''),
           width      = 6.5,
           height     = 6.5,
           pointsize  = 12,
  ...
  for(transform in c('none','linear','exponential')){
    ...
    { 
      ...
      # pdf figure:
      {
        fig.name <- paste(output.path,'/lny-vs-c-',transform,'-v0',sep='')
        tikz(paste(fig.name,'.tex',sep=''),
             width      = 4.5,
             height     = 2.5,
             pointsize  = 12,
    ...
    if(TRUE){
      ...
      {
        fig.name <- paste(output.path,'/monthly-fees-',transform,'-v0',sep='')
        tikz(paste(fig.name,'.tex',sep=''),
             width      = 3.5,
             height     = 3.5,
             pointsize  = 12,
  ...
  if(TRUE){
    ...
    {
      fig.name <- paste(output.path,'/tau-vs-theta-v0',sep='')
      tikz(paste(fig.name,'.tex',sep=''),
           width      = 5.5,
           height     = 5.5,
           pointsize  = 12,
  ...
  if(TRUE){
    ...
    fig.name <- paste(output.path,'/surplus-vs-tau-v0',sep='')
    tikz(paste(fig.name,'.tex',sep=''),
         width      = 5.5,
         height     = 5.5,
         pointsize  = 12,
    ...
    lines(tau.range,R                  ,lty=1,lwd=1,col='blue')
    lines(tau.range,TS                 ,lty=1,lwd=1,col='black')
    lines(c(tau.TS,tau.TS),c(0,max(TS)),lty=2,      col='black')
    lines(c(tau0,tau0),c(0,TS[which.min((tau.range-tau0)^2)]),lty=2,col='black')
    lines(c(tau.R,tau.R)  ,c(0,max(R)) ,lty=2,      col='blue')
  ...
  if(TRUE){
    ...
    # pdf figure: Yseries
    {
      ...
      fig.name <- paste(output.path,'/Yseries-v0',sep='')
      tikz(paste(fig.name,'.tex',sep=''),
           width      = 5.5,
           height     = 3.5,
           pointsize  = 12,
    ...
    # pdf figure: Yratio
    {
      fig.name <- paste(output.path,'/Yratio-v0',sep='')
      tikz(paste(fig.name,'.tex',sep=''),
           width      = 5.5,
           height     = 3.5,
           pointsize  = 12,
...
if(TRUE){
  ...
  for(m in 1:12){
    ...
    for(tau in tau.range){k <- k + 1
      ...
      Y.inputs$alpha.m <- 0
      Y.inputs$theta   <- (m>=5 & m<=10)*theta.hat.BC 
      Y.inputs$c       <- c + tau * (m>=6 & m<=9)
      outs             <- Y.fn(Y.inputs)
      Y.tau[k]         <- Y.tau[k] + outs$Ytm
      V.tau[k]         <- V.tau[k] - outs$Ytm/lambda.hat + tau*outs$Ytm * (m>=6 & m<=9)
      R.tau[k]         <- R.tau[k] + tau*outs$Ytm * (m>=6 & m<=9)
    }
